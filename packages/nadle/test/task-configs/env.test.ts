...
describe("env", () => {
	const exec = NewExec.createExec({ config: "env" });

	it("can inject env to process.env from object config", async () => {
		const stdout = await NewExec.getStdout(exec`firstTask`);
	...
	it("can inject env to process.env from callback config", async () => {
		const stdout = await NewExec.getStdout(exec`secondTask`);
	...
	it("should not inject env from other task", async () => {
		const stdout = await NewExec.getStdout(exec`firstTask secondTask --sequence`);
	...
	it("should not inject env from other task 2", async () => {
		const stdout = await NewExec.getStdout(exec`firstTask secondTask`);
	...
	it("should not inject env from other task 3", async () => {
		const stdout = await NewExec.getStdout(exec`firstTask secondTask --max-workers 2`);
